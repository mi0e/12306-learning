...
public class UserLoginServiceImpl implements UserLoginService {
    ...
    public UserRegisterRespDTO register(UserRegisterReqDTO requestParam) {
        // 参数校验：
        // UserRegisterParamNotNullChainHandler 非空校验
        // UserRegisterHasUsernameChainHandler 用户名唯一校验
        // UserRegisterFilterChainHandler 用户注册检查证件号是否多次注销
        abstractChainContext.handler(UserChainMarkEnum.USER_REGISTER_FILTER.name(), requestParam);
        // 乐观锁设计问题：如何解决并发问题？详情查看：https://nageoffer.com/12306/question
        // 禁止并发注册
        RLock lock = redissonClient.getLock(LOCK_USER_REGISTER + requestParam.getUsername());
        boolean tryLock = lock.tryLock();
        // 无法上锁，说明有其他线程正在注册，假设其他线程注册成功，则当前线程返回用户名非空异常
        if (!tryLock) {
    ...
    public void deletion(UserDeletionReqDTO requestParam) {
        ...
        try {
            ...
            StringRedisTemplate instance = (StringRedisTemplate) distributedCache.getInstance();
            // 写入redis
            // 查询布隆过滤器，因为布隆过滤器无法删除元素，所以需要查询复用缓存
